...
namespace Math
{
    ...
    class Vector
    {
        public:
            ...
            Vector (const Point <Data_T, Measurements>& start, const Point <Data_T, Measurements>& finish);
...
Math::Vector <Data_T, Measurements>::Vector (const Vector <Data_T, Measurements>& from):
    coords_ ()
{
    //std::copy (from.coords_.cbegin (), from.coords_.cend (), coords_);
}
...
template <typename Data_T, size_t Measurements>
Math::Vector <Data_T, Measurements>::Vector (const Point <Data_T, Measurements>& start, const Point <Data_T, Measurements>& finish):
    coords_ ()
{
    for (size_t i = 0; i < Measurements; i++)
        coords_[i] = finish[i] - start[i];
}
